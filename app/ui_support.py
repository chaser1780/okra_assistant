...
from ui_tasks import (
    build_daily_first_open_command,
    build_long_memory_approval_command,
...
def load_state(home: Path, selected: str | None = None) -> dict:
    ...
    return {
        ...
        "long_memory": read_json(home / "db" / "long_memory" / "exports" / "all_memory.json", {"items": []}),
        "long_memory_pending": read_json(home / "db" / "long_memory" / "candidates" / "pending_permanent.json", {"items": []}),
        "long_memory_market": read_json(home / "db" / "long_memory" / "market" / "regime_memory.json", {"items": []}),
        "long_memory_execution": read_json(home / "db" / "long_memory" / "execution" / "rules.json", {"items": []}),
        "daily_first_open": read_json(home / "db" / "daily_workspace" / selected / "today_decision.json", {}),
        "daily_first_open_analysis": read_json(home / "db" / "daily_workspace" / selected / "today_analysis.json", {}),
        "daily_first_open_updates": read_json(home / "db" / "daily_workspace" / selected / "memory_updates.json", {}),
        "daily_first_open_brief": read_text(home / "db" / "daily_workspace" / selected / "daily_brief.md"),
...
def build_long_memory_lines(items: list[dict], domain: str, limit: int = 12) -> list[str]:
    filtered = [item for item in items if item.get("domain") == domain]
    lines = []
    for item in filtered[:limit]:
        lines.append(
            f"{item.get('title', item.get('memory_id', ''))} | {item.get('status', '')} | "
            f"confidence={num(item.get('confidence'), 2)} | support={item.get('support_count', 0)} | "
            f"{item.get('text', '')}"
        )
    return lines


def build_pending_memory_lines(items: list[dict], limit: int = 12) -> list[str]:
    lines = []
    for item in items[:limit]:
        refs = item.get("evidence_refs", []) or []
        lines.append(
            f"{item.get('memory_id', '')} | {item.get('domain', '')}/{item.get('entity_key', '')} | "
            f"{item.get('title', '')} | confidence={num(item.get('confidence'), 2)} | evidence={len(refs)}"
        )
