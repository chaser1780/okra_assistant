...
RANGE_DAYS = {"近1月": 31, "近3月": 93, "近6月": 186, "近1年": 366, "成立以来": None}
...
def collect_quote_history(home: Path, fund_code: str, range_label: str = "成立以来") -> dict:
    stored = read_json(home / "db" / "fund_nav_history" / f"{fund_code}.json", {})
    if stored:
        ...
        return {
            ...
            "week_change_pct": _filter_points(week, range_label),
            "month_change_pct": _filter_points(month, range_label),
    ...
    for path in sorted((home / "raw" / "quotes").glob("*.json")):
        ...
        for item in payload.get("funds", []) or []:
            ...
            break
    filtered_rows = _filter_points(rows, range_label)
    return {
        "nav": filtered_rows,
        "nav_normalized": _normalize(filtered_rows),
        "day_change_pct": _filter_points(daily, range_label),
        "week_change_pct": _filter_points(week, range_label),
        "month_change_pct": _filter_points(month, range_label),
...
def collect_proxy_history(home: Path, fund_code: str, range_label: str = "成立以来") -> dict:
    ...
    if proxy_symbol:
        stored = read_json(home / "db" / "proxy_history" / f"{proxy_symbol}.json", {})
        if stored:
            items = stored.get("items", []) or []
    ...
    return {"day_change_pct": daily, "normalized": normalized, "source": "intraday_proxies_fallback"}
...
def collect_benchmark_history(home: Path, benchmark_key: str, range_label: str = "成立以来") -> dict:
    ...
    stored = read_json(home / "db" / "benchmark_history" / f"{benchmark_key}.json", {})
    if not stored:
    ...
    return {
        "price": price,
        "normalized": _normalize(price),
        "change_pct": _filter_points(change, range_label),
...
def collect_portfolio_history(home: Path, range_label: str = "成立以来") -> dict:
    ...
    return {
        ...
        "holding_pnl": _filter_points(pnl, range_label),
        "holding_return_pct": _filter_points(return_pct, range_label),
...
def collect_trade_markers(home: Path, fund_code: str | None = None, range_label: str = "成立以来") -> list[TradeMarker]:
    ...
    if not markers:
        return []
    filtered = _filter_points([SeriesPoint(marker.date, 0.0) for marker in markers], range_label)
